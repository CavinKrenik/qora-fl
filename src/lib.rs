...
#[cfg(feature = "python")]
// `#[pymethods]` in pyo3 0.20 expands to `impl` blocks nested inside a
// function, which trips `non_local_definitions`. The offending code is
// generated by the macro, not written here, so there is nothing to restructure;
// upgrading pyo3 is the real fix and is deliberately not done on a release
// branch. Scoped to this module so no hand-written non-local `impl` can hide
// behind it, and `unknown_lints` keeps older toolchains -- which do not know
// this lint -- compiling.
