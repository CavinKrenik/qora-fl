...
/// * [`QoraError::VerificationError`] if a valid update exceeds a valid bound.
pub fn check_norm_bound(update: &Array2<f32>, max_norm: f32) -> Result<(), QoraError> {
    ...
    let bound = max_norm as f64;

    if norm <= bound {
        Ok(())
    } else {
        // Scientific notation stays readable across the range this function
        // has to cover; `{:.4}` printed a 1e20 norm as 20 leading digits.
        Err(QoraError::VerificationError(format!(
            "Update norm {:.6e} exceeds bound {:.6e}",
            norm, bound
        )))
    }
}
...
mod tests {
    ...
    fn test_large_finite_vector_outside_bound_is_rejected() {
        ...
        assert!(matches!(
            ...
            Err(QoraError::VerificationError(_))
        ));
    }
    ...
    fn test_rejection_reports_finite_norm() {
        ...
        match check_norm_bound(&update, 1e19) {
            Err(QoraError::VerificationError(msg)) => assert!(
                !msg.contains("inf"),
                "rejection message must report the real norm (~1.41e20), got: {}",
                msg
            ),
    ...
    ///
    /// Pinned through the error message because the norm is not otherwise
    ...
    fn test_reported_norm_is_exact_at_both_extremes() {
        ...
        match check_norm_bound(&array![[1e20f32, 1e20f32]], 1.0) {
            Err(QoraError::VerificationError(msg)) => {
        ...
        match check_norm_bound(&array![[1e-25f32, 1e-25f32]], 1e-26) {
            Err(QoraError::VerificationError(msg)) => {
                assert!(msg.contains("1.414214e-25"), "got: {}", msg)
            }
            other => panic!("expected VerificationError, got {:?}", other),
        }
    }
